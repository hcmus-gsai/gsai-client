...
import { Alert, Button, Collapse, Drawer, Empty, Form, Input, Progress, Select, Skeleton, Spin, Tree } from 'antd';
...
import { useGetSubmitJsonQuery } from '@/store/api/[module]/projectApi';
import {
...
} from '@/store/api/[module]/showcaseApi';
...
export default function LectureProjQAV2() {
    ...
    const { data: lessonDetail } = useGetShowcaseLessonDetailQuery(lessonIdValue, {
        skip: !lessonIdValue,
    });

    const [createSession] = useCreateShowcaseProjectQASessionMutation();
    const [startInterview] = useStartShowcaseProjectQAInterviewMutation();
    const [respondToInterview] = useRespondShowcaseProjectQAMutation();
    const [triggerReport] = useLazyGetShowcaseProjectQAReportQuery();
    const [saveResult] = useSaveShowcaseProjectQAResultMutation();
    ...
    const [queuedNextQuestionMessage, setQueuedNextQuestionMessage] = useState<string | null>(null);
    const [queuedNextQuestionId, setQueuedNextQuestionId] = useState<string | null>(null);
    const [queuedNextQuestionIndex, setQueuedNextQuestionIndex] = useState<number | null>(null);
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            if (response.question_status === 'concluded_pass' || response.question_status === 'concluded_fail') {
                ...
                setQueuedNextQuestionMessage(null);
                setQueuedNextQuestionId(null);
                setQueuedNextQuestionIndex(null);
                setIsInputDisabled(true);
    ...
    const handleGoNextQuestion = async () => {
        ...
        try {
            if (queuedNextQuestionMessage && queuedNextQuestionId && queuedNextQuestionIndex !== null) {
                setCurrentQuestionIndex(queuedNextQuestionIndex);
                setQueuedNextQuestionMessage(null);
                setQueuedNextQuestionId(null);
                setQueuedNextQuestionIndex(null);
                await resetCurrentQuestionChat(queuedNextQuestionMessage);
                return;
            }
    ...
    const scorePercent = gradingReport?.percentage || 0;
    ...
    const renderResultView = () => {
        ...
        // Each question max 2, total = num_questions*2, scale total to 10, per-question stays 0/2/1/2
            ...
            <div className="flex-1 overflow-y-auto rounded-xl border border-gray-200 bg-white p-4 md:p-5">
                <div className="text-center mb-5">
                    ...
                    <p className="text-gray-500 mt-1">
                        {gradingReport.student_name || 'Học sinh Showcase'} · Project: {lessonDetail?.lesson?.lesson_name || 'Project'}
                    </p>
                </div>
    ...
    return (
        <div className="w-full h-[calc(100vh-10rem)] flex gap-4 overflow-hidden pb-4">
            <div className="hidden md:block w-[20%] min-w-[220px] max-w-[320px]">{FileExplorerContent}</div>

            <div className="flex-1 min-w-0">{CodeEditorContent}</div>

            <div className="w-[36%] min-w-[340px] max-w-[520px] flex flex-col bg-white rounded-lg border border-gray-200 p-3">
